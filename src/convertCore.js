...
async function convertSpec(spec, target) {
  ...
  if (from === targetFamily) {
    if (targetFamily === '2.0') return { openapi: withDefaultMedia20(spec), warnings: [] };
    if (!exact || spec.openapi === exact) return { openapi: spec, warnings: [] };
    return { openapi: Object.assign({}, spec, { openapi: exact }), warnings: [] };
  }
  ...
  if (targetFamily === '3.0') {
    return { openapi: stamp(await to30(spec, from, warnings)), warnings };
  }
  if (targetFamily === '3.1') {
    return { openapi: stamp(await to31(spec, from, warnings)), warnings };
  }
  if (targetFamily === '3.2') {
    return { openapi: stamp(await to31(spec, from, warnings)), warnings };
  }
  ...
  return { openapi: withDefaultMedia20(swagger20.swagger), warnings };
}
