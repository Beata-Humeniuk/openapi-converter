...
// Every message starts with the version that produced it. Installing a .vsix
// does NOT replace the code in an already open window — until the window is
// reloaded, the command runs the previous version. Without this prefix that
// situation is indistinguishable from a bug in the new version.
function versionTag() {
...
async function applyMarkersCommand(uri) {
  ...
  // Wrapping changes the STRUCTURE of the file, so it is called out explicitly.
...
// Replaces the source file's content in the editor buffer without changing the
// format (JSON stays JSON, YAML stays YAML) or the key order — the diff shows
// only the changed fields; the user saves with Ctrl+S.
...
async function convertCommand(uri) {
  ...
  for (const [line, versions] of Object.entries(OPENAPI_VERSIONS)) {
    ...
    for (const v of versions) {
      ...
      if (v === exactFrom) {
        ...
        if (line === '2.0' || (from === '3.2' && line !== '3.2')) description += ' (lossy — a warning list will be shown)';
      }
  ...
  // Tags from descriptions (EA notes) → real fields, BEFORE the conversion:
  // consumes/produces set from tags in 2.0 then convert correctly into
  // media types under content when moving to 3.x.
  ...
  if (warnings && warnings.length) {
    const pick = await vscode.window.showWarningMessage(
      'Conversion to ' + targetPick.label + ' is lossy: ' + warnings.length + ' warnings.',
      'Show warnings'
    );
