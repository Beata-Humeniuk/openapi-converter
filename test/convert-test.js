...
(async () => {
  ...
  assert(a.exclusiveMinimum === 0 && !('minimum' in a), '3.0->3.1: boolean exclusiveMinimum -> numeric');
  ...
  assert(b.maximum === 10 && b.exclusiveMaximum === true,
    '3.1->3.0: numeric exclusiveMaximum -> maximum + boolean');
  ...
  assert(c.properties.n.minimum === 0 && c.properties.n.exclusiveMinimum === true,
    '3.1->3.0: numeric exclusiveMinimum -> minimum + boolean');
    ...
    components: {
      schemas: {
        W: {
          type: 'object',
          properties: {
            haslo: { type: 'string', writeOnly: true, description: 'Hasło do ustawienia.' },
            bezOpisu: { type: 'string', writeOnly: true }
          }
        }
      }
    }
  ...
  assert(woProps.haslo.writeOnly === undefined, '3.0->2.0: writeOnly removed (2.0 does not know it)');
